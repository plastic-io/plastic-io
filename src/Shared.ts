...
export interface NodeInterface {
     ...
     */
    context: any;
    /** The host binding handed to the set function (2.1). */
    host?: HostInterface;
    /** The execution this invocation belongs to (2.1). */
    executionId?: string;
}
/**
...
/** What one `url()` invocation produced, available once the execution has ended. */
export interface ExecutionResult {
    /** Kept for compatibility; always empty. */
    nodes: Node[];
    executionId?: string;
    url?: string;
    /** How the execution ended: completed, failed, cancelled or abandoned. */
    state?: string;
    reason?: string;
    duration?: number;
    hops?: number;
...
export interface SchedulerEvent {
    ...
    duration?: number;
    /** Position of this event in the scheduler's stream (2.1) */
    seq?: number;
    /** The `url()` invocation this event belongs to (2.1) */
    executionId?: string;
    /** The edge traversal this event belongs to (2.1) */
    spanId?: string;
    /** The edge traversal that caused this one (2.1) */
    parentSpanId?: string;
}
/**
 * What node code can reach besides its edges (2.1).  The default host offers
 * the execution's identity, its cancellation state, and `emit` for custom
 * observations; embedders extend it through {@link SchedulerOptions.host}.
 */
export interface HostInterface {
    executionId: string;
    /** True once the execution was cancelled or ran out of budget. */
    readonly cancelled: boolean;
    /** An AbortSignal tied to cancellation, when the platform has one. */
    signal?: any;
    /** Throws ExecutionCancelled when the execution should stop. */
    throwIfCancelled(): void;
    /** Emit a custom observation (`observation` event), counted against the budget. */
    emit(kind: string, data?: any): void;
    now(): number;
    random(): number;
    [key: string]: any;
}
/** Emitted by `host.emit`. */
export interface ObservationEvent extends SchedulerEvent {
    kind: string;
    data?: any;
    nodeId?: string;
    graphId?: string;
}
/** Runs before a value enters a node (`onInput`) or leaves it (`onOutput`).  Throw to report a contract violation. */
export type ContractHook = (info: {node: Node; field: string; value: any; connector?: Connector; executionId: string}) => void;
/** Scheduler-wide settings (2.1).  Everything is optional; the defaults reproduce 2.0 behaviour with completion tracking added. */
export interface SchedulerOptions {
    /** Limits applied to every execution unless `invoke` overrides them. */
    budget?: any;
    /** Extra members for the `host` binding, or a factory called per node invocation. */
    host?: {[key: string]: any} | ((info: {execution: any; nodeInterface: NodeInterface}) => {[key: string]: any});
    /**
     * `direct` (default) hands the node's source to `AsyncFunction` as written,
     * so top-level `await` and every syntax the runtime supports work; `meriyah`
     * keeps the 2.0 parse-and-regenerate path.
     */
    compile?: "direct" | "meriyah";
    onInput?: ContractHook;
...
export interface EdgeError extends SchedulerEvent {
    ...
    nodeId?: string;
    /** `err.toString()`, present at every site since 2.1 */
    message?: string;
    graphId?: string;
    field?: string;
    value?: any;
    /** The output edge whose delivery failed */
    edgeField?: string;
    connectorId?: string;
    url?: string;
    /** `BUDGET_EXCEEDED` or `CONTRACT_VIOLATION` (2.1) */
    code?: string;
    dimension?: string;
    used?: number;
    limit?: number;
...
/** Which execution and span an event belongs to (2.1). */
